...
function dirsOf(type) {
  if (type === "background") return [USER_BACKGROUND_DIR(), BUILTIN_BACKGROUND_DIR()];
  if (type === "music") return [USER_MUSIC_DIR(), BUILTIN_MUSIC_DIR()];
  if (type === "video") return [USER_VIDEO_DIR(), BUILTIN_VIDEO_DIR()];
  return [];
}
