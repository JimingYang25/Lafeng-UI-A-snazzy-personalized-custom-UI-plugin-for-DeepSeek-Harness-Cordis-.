...
// 用户开发目录（当前表情包实际来源），作为兜底，移动项目后可通过 ~/.dsh/la-feng/emoji 或内置目录继续用
const DEV_EMOJI_DIR = () => join("D:\\Desktop", "dsh-client-ui-la-feng", "emoji");
...
function apply(ctx) {
  ...
  if (systemPrompt !== undefined) {
    ...
    systemPrompt.section({
      ...
      text: () => {
        ...
        return "【表情包】\n济铭收藏了一组「拉风」表情包（以 AI 角色为原型的动漫表情，存放在 la-feng 插件的 emoji 目录）。你可以调用 emoji_pick 工具挑选表情，返回的 markdown 图片引用可直接嵌入回复（绝对 http URL，浏览器会自动加载，尺寸由用户在设置里调整）。当前使用频率设定为：" + freq + "。" + freqText;
      }
    });
  } else {
  ...
  if (tools !== undefined) {
    ctx.effect(() => tools.register({
      name: "emoji_pick",
      description: "从济铭收藏的「拉风」表情包（以 AI 角色为原型的动漫表情，存放于 la-feng 插件 emoji 目录）挑选表情，返回可直接嵌入回复的 Markdown 图片引用（绝对 http URL）。适合在表达开心、吃醋、撒娇、想念、道歉等情绪时点缀使用，使用频率请遵循【表情包】提示中的设定。",
      parameters: {
        type: "object",
        properties: {
